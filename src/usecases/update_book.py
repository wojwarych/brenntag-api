...
async def update_books_price_and_rating(
    ...
    async with uow:
        try:
            ...
            if book_orm:
                logger.debug(
                    f"Old price and rating: price={book_orm.price}, rating={book_orm.rating}"  # noqa: E501
                )
                book_entity = Book.from_orm(book_orm)
                book_entity.update_price_and_rating(price_and_rating)
                book_orm = await uow.books_repo.update_item(
                    book_orm, book_entity
                )  # noqa: E501
                if book_orm:
                    await uow.commit()
                    logger.info(
                        f"Successfully updated price and rating for book: {book_orm}, price={book_orm.price}, rating={book_orm.rating}"  # noqa: E501
                    )
                    return book_entity
                else:
                    raise Exception()
